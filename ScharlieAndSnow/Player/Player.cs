    ...
    class Player
    {
        ...
        */
            ...
            for (int i = 0; i < pressedKeys.Length; ++i)
            {
                ...
                if (pressedKeys[i] == PlayerManager.validKeys[_playerId][4])
                {
                    if (amountSnowball > 0 && timer >= attackSpeed)
                    {
                        ...
                        if (_currentDirection == Direction.Right)
                            MapStuff.Instance.partCollHandler.AddParticle(_pos + new Vector2(textureWidth + 5,0), 30, snowballSize, snowballMove,snowballDamage);
                        else
        ...
        void CheckCollision(Vector2 _mov)
        {
            ...
            {
                Vector2 newPosition = sprite.Position + _mov;
